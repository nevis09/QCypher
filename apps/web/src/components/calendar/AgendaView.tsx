  ...
  return (
    <>
      <div className="space-y-4">
        {groups.length === 0 && (
          <div className="rounded-2xl border border-[hsl(var(--border))] bg-white dark:bg-[hsl(var(--muted))] p-10 text-center">
        ...
        {groups.map(({ date, events: dayEvents }) => (
          <div key={format(date, 'yyyy-MM-dd')}>
            <p className={`text-xs font-semibold uppercase tracking-wide mb-2 ${isToday(date) ? 'text-accent' : 'text-[hsl(var(--muted-foreground))]'}`}>
              {dayLabel(date)}
            </p>
            <div className="space-y-2">
              {dayEvents.map(ev => (
                <button
                  ...
                  className="w-full flex items-start gap-3 bg-white dark:bg-[hsl(var(--muted))] rounded-2xl border border-[hsl(var(--border))] shadow-soft px-4 py-3 text-left hover:bg-[hsl(var(--muted))]/60 transition-colors touch-target"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {ev.description && (
                      <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1 line-clamp-2">{ev.description}</p>
                    )}
                  </div>
                </button>
              ))}
      ...
      >
        ...
        <span className="hidden md:inline text-sm font-medium">New event</span>
      </button>
