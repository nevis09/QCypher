...
-- migration only adds:
--   1. public.user_role() — mirrors auth.tenant_id(), reads
...
-- staleness caveat as auth.tenant_id() — a role change via
...
create policy "contacts: tenant isolation insert"
  ...
  with check (tenant_id = auth.tenant_id() and public.user_role() <> 'read_only');

drop policy if exists "contacts: tenant isolation update" on contacts;
create policy "contacts: tenant isolation update"
  on contacts for update
  using (tenant_id = auth.tenant_id() and public.user_role() <> 'read_only')
  with check (tenant_id = auth.tenant_id() and public.user_role() <> 'read_only');
